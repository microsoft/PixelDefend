# copied from https://github.com/carlini/nn_robust_attacks
...
class CarliniLi:
	...
	def gradient_descent(self, sess, model):
		...
		output = model.predict((newimg + 0.5) * 255.0)
		orig_output = model.predict((tf.tanh(timg) / 2 + 0.5) * 255.0)
	...
	def attack(self, imgs, targets):
		"""
		Perform the L_infinity attack on the given images for the given targets.
		...
		for img, target in zip(imgs, targets):
			count += 1
			print("Performing CW attack on {}th image".format(count))
