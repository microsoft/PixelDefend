...
"""
A mostly copied but slightly modified version of OpenAI's pixel_cnn_pp/nn.py from https://github.com/openai/pixel-cnn
"""
...
def discretized_mix_logistic_loss(x, l, sum_all=True):
    ...
    x = tf.reshape(x, xs + [1]) + tf.zeros(
        xs + [nr_mix])  # here and below: getting the means and adjusting them based on preceding sub-pixels
...
def sample_from_discretized_mix_logistic(l, nr_mix, seed=None):
    ...
    sel = tf.one_hot(tf.argmax(logit_probs - tf.log(
        -tf.log(tf.random_uniform(logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5, seed=seed))), 3),
...
def compute_xent(x, l, data_set='cifar'):
    recover_img = tf.cast(x * 127.5 + 127.5, dtype=tf.int32)
...
def decode_from_softmax_with_box_constraint_pixel_full(x, l, eps):
    '''
    Choose adaptive eps based on the log-likelihood of the pixel from the given image.
    '''
    # channel 0
    x0 = x
    recover_img0 = tf.cast(x0 * 127.5 + 127.5, dtype=tf.int32)
    recover_label0 = tf.one_hot(recover_img0, depth=256)
    logits0 = mix_logistic_to_logits(x0, l)
    xent0 = tf.nn.softmax_cross_entropy_with_logits(logits=logits0, labels=recover_label0)  # (batch_size, 1, 1, 3)
    xent0 = tf.squeeze(xent0[..., 0])  # (batch_size,)
    channel_eps0 = tf.cast(tf.clip_by_value(0.5 * eps * xent0, 0.0, 32.0), dtype=tf.int32)
    lb0 = tf.maximum(recover_img0[:, :, :, 0] - channel_eps0[:, None, None], 0)
    ub0 = tf.minimum(recover_img0[:, :, :, 0] + channel_eps0[:, None, None], 255)
    template0 = tf.range(0, 256, dtype=tf.int32) + tf.zeros_like(recover_img0[:, :, :, 0], dtype=tf.int32)[..., None]
    lb0 = lb0[..., None] + tf.zeros_like(template0, dtype=tf.int32)
    ub0 = ub0[..., None] + tf.zeros_like(template0, dtype=tf.int32)
    template0 = tf.cast(tf.logical_or(tf.less(template0, lb0), tf.greater(template0, ub0)), tf.float32)
    logits0 = logits0[:, :, :, 0, :] - template0 * 1e30
    x0 = (tf.cast(tf.argmax(logits0, axis=-1), dtype=tf.float32) - 127.5) / 127.5

    # channel 1
    x1 = tf.stack([x0, x[..., 1], x[..., 2]], axis=3)
    recover_img1 = tf.cast(x1 * 127.5 + 127.5, dtype=tf.int32)
    recover_label1 = tf.one_hot(recover_img1, depth=256)
    logits1 = mix_logistic_to_logits(x1, l)
    xent1 = tf.nn.softmax_cross_entropy_with_logits(logits=logits1, labels=recover_label1)  # (batch_size, 1, 1, 3)
    xent1 = tf.squeeze(xent1[..., 1])  # (batch_size,)
    channel_eps1 = tf.cast(tf.clip_by_value(0.5 * eps * xent1, 0.0, 32.0), dtype=tf.int32)
    lb1 = tf.maximum(recover_img1[:, :, :, 1] - channel_eps1[:, None, None], 0)
    ub1 = tf.minimum(recover_img1[:, :, :, 1] + channel_eps1[:, None, None], 255)
    template1 = tf.range(0, 256, dtype=tf.int32) + tf.zeros_like(recover_img1[:, :, :, 1], dtype=tf.int32)[..., None]
    lb1 = lb1[..., None] + tf.zeros_like(template1, dtype=tf.int32)
    ub1 = ub1[..., None] + tf.zeros_like(template1, dtype=tf.int32)
    template1 = tf.cast(tf.logical_or(tf.less(template1, lb1), tf.greater(template1, ub1)), tf.float32)
    logits1 = logits1[:, :, :, 1, :] - template1 * 1e30
    x1 = (tf.cast(tf.argmax(logits1, axis=-1), dtype=tf.float32) - 127.5) / 127.5

    # channel 2
    x2 = tf.stack([x0, x1, x[..., 2]], axis=3)
    recover_img2 = tf.cast(x2 * 127.5 + 127.5, dtype=tf.int32)
    recover_label2 = tf.one_hot(recover_img2, depth=256)
    logits2 = mix_logistic_to_logits(x2, l)
    xent2 = tf.nn.softmax_cross_entropy_with_logits(logits=logits2, labels=recover_label2)  # (batch_size, 1, 1, 3)
    xent2 = tf.squeeze(xent2[..., 2])  # (batch_size,)
    channel_eps2 = tf.cast(tf.clip_by_value(0.5 * eps * xent2, 0.0, 32.0), dtype=tf.int32)
    lb2 = tf.maximum(recover_img2[:, :, :, 2] - channel_eps2[:, None, None], 0)
    ub2 = tf.minimum(recover_img2[:, :, :, 2] + channel_eps2[:, None, None], 255)
    template2 = tf.range(0, 256, dtype=tf.int32) + tf.zeros_like(recover_img2[:, :, :, 2], dtype=tf.int32)[..., None]
    lb2 = lb2[..., None] + tf.zeros_like(template2, dtype=tf.int32)
    ub2 = ub2[..., None] + tf.zeros_like(template2, dtype=tf.int32)
    template2 = tf.cast(tf.logical_or(tf.less(template2, lb2), tf.greater(template2, ub2)), tf.float32)
    logits2 = logits2[:, :, :, 2, :] - template2 * 1e30
    x2 = (tf.cast(tf.argmax(logits2, axis=-1), dtype=tf.float32) - 127.5) / 127.5

    return tf.stack([x0, x1, x2], axis=3)


def sampling_from_softmax_with_box_constraint(x, l, eps, T=1.):
    '''
    Greedy decoding. Equivalent to beam search with size = 1
    Note: The output is a unnormalized image
    '''
