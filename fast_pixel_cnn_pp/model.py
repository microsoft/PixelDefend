...
def model_spec(row_input,
    ...
    with arg_scope(
        ...
        with arg_scope(
            ...
            xent = nn.compute_xent(ref_img, x_out, data_set=data_set) / np.log(2.0)
            ...
            if not denoise:
                sample = nn.sample_from_softmax(ref_img, x_out, data_set=data_set)
            else:
                sample = nn.decode_from_softmax_with_box_constraint(ref_img, x_out, adpt_eps, data_set=data_set)
