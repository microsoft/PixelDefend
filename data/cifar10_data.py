...
def unpickle(file):
    ...
    return {'x': d['data'].reshape((-1, 3, 32, 32)), 'y': np.array(d['labels']).astype(np.uint8)}
...
def load_without_download(data_dir, subset='train'):
    if subset == 'train':
        train_data = [unpickle(os.path.join(data_dir, 'data_batch_' + str(i))) for i in
                      range(1, 6)]
        trainx = np.concatenate([d['x'] for d in train_data], axis=0)
        trainy = np.concatenate([d['y'] for d in train_data], axis=0)
        return trainx, trainy
...
class DataLoader(object):
    ...
    def __init__(self, data_dir, subset, batch_size, rng=None, shuffle=False, return_labels=False, download=False):
        """
        ...
        if download:
            ...
            self.data, self.labels = load_without_download(data_dir, subset=subset)
